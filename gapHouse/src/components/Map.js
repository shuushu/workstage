...
let populations2;
let g = window;
const POINT_NAME = ["수원 장안구", "수원 권선구", "수원 팔달구", "수원 영통구", "성남 수정구", "성남 중원구", "성남 분당구", "의정부시", "안양시만안구", "안양시동안구", "부천시", "광명시", "평택시", "동두천시", "안산상록구", "안산단원구", "덕양구", "일산동구", "일산서구", "과천시", "구리시", "남양주시", "오산시", "시흥시", "군포시", "의왕시", "하남시", "용인처인구", "용인기흥구", "용인수지구", "파주시", "이천시", "안성시", "김포시", "화성시", "광주시", "양주시", "포천시", "여주시", "연천군", "가평군", "양평군", "종로구", "중구", "용산구", "성동구", "광진구", "동대문구", "중랑구", "성북구", "강북구", "도봉구", "노원구", "은평구", "서대문구", "마포구", "양천구", "강서구", "구로구", "금천구", "영등포구", "동작구", "관악구", "서초구", "강남구", "송파구", "강동구", "중구", "동구", "미추홀구", "연수구", "남동구", "부평구", "계양구", "서구", "강화군"];
function delay() {
...
var countryStrokeColor = am4core.color("#000");
...
let currentPolygon,
    ...
    currentTypeName,
    sliderAnimation,
    polygonSeries, mapChart, bubbleSeries,
    perCapita = false,
    // for an easier access by key
    colors = { active: activeColor, confirmed: confirmedColor, recovered: recoveredColor, deaths: deathsColor },
    max = { confirmed: 30, recovered: 30, deaths: 30, acrive: 30 },
    maxPC = { confirmed: 10, recovered: 10, deaths: 10, active: 10 },
...
function setToOrigin(key) {
    ...
    switch (key) {
        ...
        case '김용현': strName = 'active';
            break;
        
        case '강선범': strName = 'confirmed';
            break;
        
        case '진현철': strName = 'recovered';
            break;
        
        case '육정순': strName = 'deaths';
            break;
        default: strName = key;
    }
    return strName
...
function setToCustom(key) {
    let strName;
    switch (key) {
        case 'active': strName = '김용현';
            break;

        case 'confirmed': strName = '강선범';
            break;

        case 'recovered': strName = '진현철';
            break;

        case 'deaths': strName = '육정순';
            break;
        default: strName = key;
...
function sideBottom() {
    var sbContainer = am4core.create("sideBottom", am4core.Container);
    sbContainer.width = am4core.percent(100);
    sbContainer.height = am4core.percent(100);
    ...
    var buttonsAndChartContainer = sbContainer.createChild(am4core.Container);
    ...
    // 하단 타이라인 슬라이드
    // 슬라이드 버튼
     var btnContainer = am4core.create("playBtn", am4core.Container);
     var sliderContainer = btnContainer.createChild(am4core.Container);
    ...
    slider.events.on("rangechanged", function (event) {
        var index = Math.round((g.point_area.length - 1) * slider.start);
    ...
    console.log(playButton.background.states)
    ...
    lineChart.data = JSON.parse(JSON.stringify(g.timeline));
    ...
    // create series
    // active, confirmed~ 값은 데이터의 key값이랑 매칭됨
    var activeSeries = addSeries("김용현", activeColor);
    // active series is visible initially
    activeSeries.tooltip.disabled = true;
    activeSeries.hidden = false;

    var confirmedSeries = addSeries("강선범", confirmedColor);
    var recoveredSeries = addSeries("진현철", recoveredColor);
    var deathsSeries = addSeries("육정순", deathsColor);
    ...
    function createColumnSeries() {
        columnSeries = {}
        // active, confirmed~ 값은 데이터의 key값이랑 매칭됨
        columnSeries.active = addColumnSeries("김용현", activeColor);
        columnSeries.active.events.on("validated", function () {
            updateColumnsFill();
        })

        columnSeries.confirmed = addColumnSeries("강선범", confirmedColor);
        columnSeries.recovered = addColumnSeries("진현철", recoveredColor);
        columnSeries.deaths = addColumnSeries("육정순", deathsColor);
    }
    ...
    // BUTTONS
    // create buttons
    var activeButton = addButton("김용현", activeColor);
    var confirmedButton = addButton("강선범", confirmedColor);
    var recoveredButton = addButton("진현철", recoveredColor);
    var deathsButton = addButton("육정순", deathsColor);
    ...
    function changeDataType(name) {
        var custom = setToCustom(name);
        var name = setToOrigin(name);
        ...
        bubbleSeries.mapImages.template.tooltipText = "[bold]{custom}: {value}[/] [font-size:11px]\n" + currentTypeName;
        ...
        //bubbleSeries.heatRules.getIndex(0).max = 30;
    ...
    function updateTotals(index) {
        if (!isNaN(index)) {            
            ...
            for (var key in buttons) {
                let name = setToCustom(key);
                var count = Number(lineChart.data[index][name])
                if (!isNaN(count)) {                    
                    // 버튼 이름
                    buttons[key].label.text = capitalizeFirstLetter(name) + ": " + numberFormatter.format(count, '#,###');
                }
            }
            currentIndex = index;
        }
...
function amInit() {
    ...
    // last date of the data
    lastDate = new Date(g.timeline[g.timeline.length - 1].date);

    //////////////////////////////////////////////////////////////////////////////
    // PREPARE DATA
    //////////////////////////////////////////////////////////////////////////////
    var list = g.point_area[g.point_area.length - 1].list;
    for (var i = 0; i < list.length; i++) {
        var country = list[i]
    ...
    mapChart = container.createChild(am4maps.MapChart);    
    ...
    mapChart.geodata = geo;
    mapChart.reverseGeodata = true;
    // Set projection
    ...
    mapChart.backgroundSeries.mapPolygons.template.polygon.fill = am4core.color('#000');
    ...
    var labelSeries = mapChart.series.push(new am4maps.MapImageSeries());
    ...
    // Bubble series
    bubbleSeries = mapChart.series.push(new am4maps.MapImageSeries());
    bubbleSeries.data = JSON.parse(JSON.stringify(mapData));
    bubbleSeries.dataFields.value = "김용현";
