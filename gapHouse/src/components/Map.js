...
// LAYOUT & CHARTS
function initCanvas(nodeName) {
  // main container
  // https://www.amcharts.com/docs/v4/concepts/svg-engine/containers/
  container = am4core.create(nodeName, am4core.Container);
...
// map feature
function drawMapFeature(mapData) {
  ...
  // 맵라벨 옵션
  let labelSeries = mapChart.series.push(new am4maps.MapImageSeries());
...
function drawBubbleTooltip() {
  ...
  bubbleSeries.tooltip.background.fillOpacity = 0.2;
  bubbleSeries.tooltip.background.fill = am4core.color("black");
}
// PREPARE DATA
...
function amInit() {
  ...
  const mapData = setPaperData();
  initCanvas("chartdiv");
  const polygonTemplate = drawMapFeature(mapData);
  ...
  const imageTemplate = drawBubble(mapData);
  // adjust tooltip
  drawBubbleTooltip();
