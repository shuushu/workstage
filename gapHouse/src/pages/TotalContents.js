...
export default function TotalCont() {
  ...
  useEffect(() => {
    callSheet('1qoH5xrmgGTaZih3uegresvbRHcUaLPgMlsrwbR7lZd4', 'area').then(res => {
  ...
  const nodes = [
    ...
    <p className="parag">
      김 씨가 매입한 주택은 서울, 인천, 경기 전역에 걸쳐 있는데요, 특히 서울 강서구와 금천구, 경기 부천시 등 서부권에 집중돼 있습니다.
    </p>,
    <p className="parag">
      원의 크기가 클 수록 해당 지역에 갖고 있는 집이 많다는 의미입니다.
    </p>,
    <div className="chip-wrap">
      <Chip avatar={<Avatar>653</Avatar>} label="전체" onClick={() => handleClick('전체')} />
      <Chip avatar={<Avatar>490</Avatar>} label="서울" onClick={() => handleClick('서울')} />
      <Chip avatar={<Avatar>126</Avatar>} label="경기도" onClick={() => handleClick('경기도')} />
      <Chip avatar={<Avatar>37</Avatar>} label="인천" onClick={() => handleClick('인천')} />
    </div>
  ]
  ...
  return <div id="cont-total" className="map-contents">
    {ua() ? 
      ...
      >
        ...
        <SwiperSlide className={`slide-items1`}>{nodes[1]}

        </SwiperSlide>
        <SwiperSlide className={`slide-items1`}>{nodes[2]}
