export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["html", "name"], function (html, name) {
    return (
      html`<b style="display:block;text-align:center;border:solid 1px red;line-height:33px;">${name}`
    )
  });
  main.variable(observer("canvas")).define("canvas",
    ["DOM", "width", "height", "d3", "sphere", "land", "borders", "countries", "mutable name", "tilt", "Versor"],
    async function* (DOM, width, height, d3, sphere, land, borders, countries, $0, tilt, Versor) {
      ...
      function render(country, arc) {
        ...
        context.beginPath(), path(country), context.fillStyle = optGlobalGeo.backgroundColor, context.fill();
        context.beginPath(), path(borders), context.strokeStyle = "#ccc", context.lineWidth = 0.5, context.stroke();
        context.beginPath(), path(sphere), context.strokeStyle = optGlobalGeo.strokeColor, context.lineWidth = 0.5, context.stroke();
  ...
  main.variable(observer("height")).define("height", ["width"], function (width) {
    return (
      Math.min(width, optGlobalGeo.height)
    )
  });
